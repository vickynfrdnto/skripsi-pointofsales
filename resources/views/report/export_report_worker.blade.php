				...
				<tbody>
					@foreach($supplies as $supply)
							<tr>
								...
								<td class="text-left text-12 txt-dark">Rp. {{ number_format($supply->harga_beli) }}</td>
								@php
								$total = $supply->jumlah * $supply->harga_beli;
								$pengeluaran += $total;
								$jml_aktv_pengeluaran += 1;
								@endphp
								<td class="text-left text-12 txt-green">Rp. {{ number_format($total) }}</td>
							</tr>
					@endforeach
				</tbody>
			</table>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					@foreach($transactions as $transaction)
						@if ($loop->first || $transaction->kode_transaksi !== $transactions[$loop->index - 1]->kode_transaksi)
							<tr>
								...
								<td>
									...
									<span class="txt-green text-12 d-block">Rp. {{ number_format($transaction->total) }}</span>
								</td>
								@php
								...
								<td>
									...
									<span class="txt-dark2 text-12 d-block">Rp. {{ number_format($transaction->bayar) }}</span>
								</td>
								<td>
									<span class="text-10 txt-light d-block">Kembali</span>
									<span class="txt-dark text-12 d-block">Rp. {{ number_format($transaction->kembali) }}</span>
								</td>
							</tr>
						@endif
					@endforeach
