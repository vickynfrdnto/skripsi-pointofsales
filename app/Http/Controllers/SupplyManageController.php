...
use App\Exports\ExportExcelPasok;
...
class SupplyManageController extends Controller
{
    ...
    public function exportexcelPasok(Request $req)
    {
        $id_account = Auth::id();
        ...
        if ($check_access->kelola_laporan == 1) {
            ...
            foreach ($supplies as $supply) {
                $data[] = [
                    ...
                    'pemasok' => $supply->pemasok,
                ];
            }
    
            // Generate nama file Excel
            $tgl_akhir = date('Y-m-d', strtotime('-1 day'));
            $fileName = 'Laporan pasok ' . $tgl_awal . ' - ' . $tgl_akhir . '.xlsx';
    
            if ($supplies->isEmpty()) {
                Session::flash('import_failed', 'Tidak ada pasok pada tanggal yang dipilih');
                return back();
            } else {
                return Excel::download(new ExportExcelPasok($data), $fileName);
            }
        } else {
            return back();
        }
