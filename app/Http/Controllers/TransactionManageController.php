...
class TransactionManageController extends Controller
{
    ...
    public function viewTransaction()
    {
        ...
        if($check_access->transaksi == 1){
            ...
            return view('transaction.transaction', compact('products', 'supply_system', 'pelanggans'));
        }else{
            return back();
        }
    }
    ...
    public function transactionProduct($id)
    {
        ...
        ->first();
        if($check_access->transaksi == 1){
        	$product = Product::where('nama_barang', '=', $id)
        	->first();
        	$supply_system = Supply_system::first();
   ...
   public function transactionProcess(Request $req)
{
    ...
    if ($check_access->transaksi == 1) {
        ...
        for ($i = 0; $i < $jml_barang; $i++) {
            ...
            $transaction->merek = $product_data->merek;
        ...
        if ($check_supply_system->status == true) {
            for ($j = 0; $j < $jml_barang; $j++) {
                ...
                // Tambahkan barang keluar ke tabel barang_movements
                DB::table('barang_movements')->insert([
                    'kode_barang' => $req->kode_barang[$j],
                    'jenis' => 'keluar',
                    'jumlah' => $req->jumlah_barang[$j],
                    'keterangan' => 'Barang terjual via transaksi ' . $req->kode_transaksi,
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);

                $product_status = Product::where('kode_barang', '=', $req->kode_barang[$j])->first();
